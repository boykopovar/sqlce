"""
    python sdf_d1.py test_35_mode1.sdf --password secret123 --verify
    python sdf_d1.py test_35_mode1.sdf --password secret123 --out decrypted.sdf
"""
...
def _crypt_derive_key_sha1_3des(base_hash: bytes, extra: bytes) -> bytes:
...
def derive_key(password: str, key_param: bytes) -> bytes:
    ...
    return _crypt_derive_key_sha1_3des(base_hash, key_param)


def des3_cbc_zero_iv(key: bytes):
    return Cipher(algorithms.TripleDES(key), modes.CBC(b"\x00" * 8))


def verify_password(page0: bytes, password: str) -> bool:
    key_param = page0[188:192]
    key = derive_key(password, key_param)
    dec = des3_cbc_zero_iv(key).decryptor()
...
def main():
    ...
    if mode != 1:
        print(f"warning: page0 reports mode={mode}, this script is for mode 1")
