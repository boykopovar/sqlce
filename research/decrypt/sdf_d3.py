"""
    python sdf_d.py protected.sdf --password secret123 --verify
    python sdf_d.py protected.sdf --password secret123 --out decrypted.sdf
"""
...
def derive_key(password: str, key_param: bytes) -> bytes:
    digest = hashlib.sha256(password.encode("utf-16-le") + key_param).digest()
    return digest[:16]
