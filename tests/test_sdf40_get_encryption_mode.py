...
def test_sdf40_engine_default_static_mode_is_aes256_sha512(sdf_dir: Path) -> None:
    ...
    assert SdfDatabase.get_encryption_mode_from_file(str(path)) == EncryptionMode.AES256_SHA512
...
def test_sdf40_platform_default_static_mode_is_aes128_sha256(sdf_dir: Path) -> None:
    ...
    assert SdfDatabase.get_encryption_mode_from_file(str(path)) == EncryptionMode.AES128_SHA256
